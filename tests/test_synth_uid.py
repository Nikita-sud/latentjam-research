...
def test_algorithm_reproduces_real_auxio_uids_byte_for_byte():
    ...
    # Deterministic: exactly 79/136 reproduce (the rest carried album/date/track not in
    # the CSV). Pinned so any regression in the directory-fallback logic fails loudly.
    assert matched == 79, f"expected exactly 79 byte-for-byte matches, got {matched}/{len(auxio)}"
...
# Real-data (CachedFileData -> real songUid) validation lives in test_synth_cachedfile.py,
# backed by the committed fixture tests/fixtures/cachedfile_uid_vectors.json.
